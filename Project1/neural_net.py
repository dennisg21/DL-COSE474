...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    a1 = np.maximum(0, np.dot(X, W1) + b1) #N * hidden_size
    a2 = np.dot(a1, W2) + b2 #N * C
    scores = a2.copy()
    ...
    dscore = prob# N*C
    dscore[range(N), y] -= 1
    grads['W2'] = np.dot(a1.T, dscore) / N + 2 * reg * W2 # H*C
    grads['b2'] = np.sum(dscore, axis=0) / N # C
 
    da1 = np.dot(dscore, W2.T) # N*H
    da1[a1 < 0.00001] = 0
    grads['W1'] = np.dot(X.T, da1) / N + 2 * reg * W1 # D*H
    grads['b1'] = np.sum(da1,axis = 0) / N # H
